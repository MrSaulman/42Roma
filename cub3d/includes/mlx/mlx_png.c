...
int	mipng_data(void *img, unsigned char *dat, png_info_t *pi)
{
	...
	b_pos = 0;
	if (!(buffer = malloc((long long)pi->width * (long long)pi->height
				* (long long)pi->bpp + pi->height)))
		return (ERR_MALLOC);
	...
	while (mipng_is_type(dat, "IDAT"))
	{
		...
		while (z_strm.avail_out == 0)
		{
			...
			if (b_pos + Z_CHUNK - z_strm.avail_out > pi->width * pi->height
				* pi->bpp + pi->height)
			{
				inflateEnd(&z_strm);
				free(buffer);
...
unsigned long		crc_table[256] = {0, 0x77073096, 0xee0e612c, 0x990951ba,
			0x76dc419, 0x706af48f, 0xe963a535, 0x9e6495a3, 0xedb8832,
			0x79dcb8a4, 0xe0d5e91e, 0x97d2d988, 0x9b64c2b, 0x7eb17cbd,
			...
			0x9abfb3b6, 0x3b6e20c, 0x74b1d29a, 0xead54739, 0x9dd277af,
			0x4db2615, 0x73dc1683, 0xe3630b12, 0x94643b84, 0xd6d6a3e,
			0x7a6a5aa8, 0xe40ecf0b, 0x9309ff9d, 0xa00ae27, 0x7d079eb1,
			...
			0x756aa39c, 0x26d930a, 0x9c0906a9, 0xeb0e363f, 0x72076785,
			0x5005713, 0x95bf4a82, 0xe2b87a14, 0x7bb12bae, 0xcb61b38,
...
void	*mlx_int_parse_png(void *xvar, unsigned char *fptr, int size,
...
void	*mlx_png_file_to_image(void *xvar, char *file, int *width, int *height)
{
	...
	if ((fd = open(file, O_RDONLY)) == -1 || (size = lseek(fd, 0, SEEK_END)) ==
		-1 ||
		(ptr = mmap(0, size, PROT_READ, MAP_PRIVATE, fd,
					0)) == (void *)MAP_FAILED)
	{
		if (fd >= 0)
			close(fd);

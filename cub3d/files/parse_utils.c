...
/*   Updated: 2022/08/26 16:45:34 by gcrocett         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_par(t_alls *sts)
{
	...
	sts->ceiling_color.r = -1;
	sts->ceiling_color.g = -1;
...
int	check_ext(t_alls *st, char *ext)
{
	...
	if (i > 4 && st->paths.map_path[i - 1] == ext[2]
		&& st->paths.map_path[i - 2] == ext[1]
		&& st->paths.map_path[i - 3] == ext[0]
		&& st->paths.map_path[i - 4] == '.')
	{
		return (1);
	}
	else
